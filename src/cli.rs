...
//! Daemon command line: `--dump-config`, `--sensors-once`, `--render-once`,
//! `--daemon`, `--send-test`. Shared with GUI tools via the library root.

use std::path::PathBuf;

use crate::config::{load_app_config, load_theme};
use crate::daemon;
use crate::logger;
use crate::sensors::{NetSelection, Provider, SlowCtx};
...
fn parse_args() -> Result<Args, String> {
    // Minimal hand-rolled parser: keeps the daemon binary free of clap bloat
    // (arg parsing runs once at startup).
    let mut args = Args {
...
pub fn run() {
    ...
    // Raw STATS keys present (proves default.yaml merge worked).
...
fn render_once(cfg: &crate::config::AppConfig, theme: &crate::config::Theme) {
